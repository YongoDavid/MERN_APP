...
export const fetchReviews = (n, v) => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get(`${REACT_APP_API_URL}/review`, {
        params: {
          page: v ?? 1,
          limit: 20
        }
...
export const approveReview = review => {
  return async (dispatch, getState) => {
    try {
      await axios.put(`${REACT_APP_API_URL}/review/approve/${review._id}`);
...
export const rejectReview = review => {
  return async (dispatch, getState) => {
    try {
      await axios.put(`${REACT_APP_API_URL}/review/reject/${review._id}`);
...
export const deleteReview = id => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.delete(`${REACT_APP_API_URL}/review/delete/${id}`);
...
export const fetchProductReviews = slug => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.get(`${REACT_APP_API_URL}/review/${slug}`);
...
export const addProductReview = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(`${REACT_APP_API_URL}/review/add`, {
        ...santizedReview,
        isRecommended: review.isRecommended.value
      });
