...
export const addMerchant = (isBack = false) => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(`${REACT_APP_API_URL}/merchant/add`, merchant);
...
export const fetchMerchants = (n, v) => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get(`${REACT_APP_API_URL}/merchant`, {
        params: {
          page: v ?? 1,
          limit: 20
        }
...
export const searchMerchants = filter => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get(`${REACT_APP_API_URL}/merchant/search`, {
        params: {
          search: filter.value
        }
      });
...
export const disableMerchant = (merchant, value, search, page) => {
  return async (dispatch, getState) => {
    try {
      await axios.put(`${REACT_APP_API_URL}/merchant/${merchant._id}/active`, {
        merchant: {
          isActive: value
        }
      });
...
export const approveMerchant = (merchant, search, page) => {
  return async (dispatch, getState) => {
    try {
      await axios.put(`${REACT_APP_API_URL}/merchant/approve/${merchant._id}`);
...
export const rejectMerchant = (merchant, search, page) => {
  return async (dispatch, getState) => {
    try {
      await axios.put(`${REACT_APP_API_URL}/merchant/reject/${merchant._id}`);
...
export const merchantSignUp = token => {
  return async (dispatch, getState) => {
    try {
      ...
      await axios.post(`${REACT_APP_API_URL}/merchant/signup/${token}`, merchant);
  ...
  return async (dispatch, getState) => {
    try {
      const response = await axios.delete(
        `${REACT_APP_API_URL}/merchant/delete/${merchant._id}`
      );
